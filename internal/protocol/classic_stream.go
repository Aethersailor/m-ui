...
func (VMessModule) Capability() ProtocolCapability {
	return mountClassicCapability(vmessCapability(), "vmess", map[string]any{"vmess": domain.VMessCredential{Cipher: "auto"}})
}
...
func (TrojanModule) Capability() ProtocolCapability {
	return mountClassicCapability(trojanCapability(), "trojan", map[string]any{"trojan": domain.TrojanCredential{}})
}
